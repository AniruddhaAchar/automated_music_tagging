...
featured_play_lists = sp.featured_playlists(country='US').get('playlists').get('items')
...
def get_featured_songs():
    ...
    for featured_playlist in featured_play_lists:
        play_list_details = sp.user_playlist_tracks(playlist_id=featured_playlist.get('id'),
                                                    user=featured_playlist.get('owner').get('id'), limit=10)
...
# todo write functions to process and categorise a given audio file.
